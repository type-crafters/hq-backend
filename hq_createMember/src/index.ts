...
import { type CreateMemberRequest, type JSONResponse, type MemberItem } from "@typecrafters/hq-types";
...
const handler = async (event: APIGatewayProxyEventV2): Promise<ResponseObject> => {
    ...
    try {
        ...
        try {
            ...
            if (!permissions.includes("create:member")) {
        ...
        const { firstName, lastName, role, bio, email, profilePictureUrl, since }: CreateMemberRequest = data;
