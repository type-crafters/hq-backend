...
import { LoggerFactory, HttpResponse, HttpCode, Authenticator, ExpiredTokenError, InvalidTokenError, Cookie, EJS, Mailer } from "@typecrafters/hq-lib";
...
const handler = async (event: APIGatewayProxyEventV2) => {
    ...
    try {
        ...
        try {
            const { firstName, lastName, email, permissions }: InviteUserRequest = JSON.parse(body);

            if (!firstName || typeof firstName !== "string") throw new TypeError();
            if (!lastName || typeof lastName !== "string") throw new TypeError();
            if (!email || typeof email !== "string") throw new TypeError();
            if (!(permissions instanceof Array)) throw new TypeError();
